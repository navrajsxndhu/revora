...
// 1. Generate Engines
const enginesDir = path.join('src', 'lib', 'compliance');
mkdirp(enginesDir);
...
for (const api of apis) {
  const apiDir = path.join('src', 'app', 'api', 'compliance', api);
  mkdirp(apiDir);
...
// 3. Generate Widgets
const widgetsDir = path.join('src', 'components', 'mission-control', 'compliance');
mkdirp(widgetsDir);
...
// 4. Generate Page
const pageDir = path.join('src', 'app', 'mission-control', 'compliance');
mkdirp(pageDir);
const imports = widgets.map(w => `import { ${w} } from '@/components/mission-control/compliance/${w}';`).join('\n');
...
export default function ComplianceGovernancePage() {
  return (
    <div className="min-h-screen bg-slate-950 p-6">
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-slate-100 tracking-tight">Enterprise Compliance Governance</h1>
        <p className="text-slate-400 text-sm mt-1">Constitutional Evidence Consolidation Ledger</p>
      </div>
...
if (fs.existsSync('src/app/api/compliance')) {
  const dirs = fs.readdirSync('src/app/api/compliance');
  for (const dir of dirs) {
    if (!allGeneratedApis.has(dir) && fs.statSync(path.join('src/app/api/compliance', dir)).isDirectory()) {
      fs.rmSync(path.join('src/app/api/compliance', dir), { recursive: true, force: true });
      console.log(\`Cleaned up legacy API route: \${dir}\`);
    }
  }
}
console.log("Phase 215 files generated successfully.");
