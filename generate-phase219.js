...
// 1. Generate Engines
const enginesDir = path.join('src', 'lib', 'production');
mkdirp(enginesDir);
...
for (const api of apis) {
  const apiDir = path.join('src', 'app', 'api', 'production', api);
  mkdirp(apiDir);
...
// 3. Generate Widgets
const widgetsDir = path.join('src', 'components', 'mission-control', 'production');
mkdirp(widgetsDir);
...
// 4. Generate Page
const pageDir = path.join('src', 'app', 'mission-control', 'production');
mkdirp(pageDir);
const imports = widgets.map(w => `import { ${w} } from '@/components/mission-control/production/${w}';`).join('\n');
const components = widgets.map(w => `<${w} />`).join('\n        ');

const pageContent = `import React from 'react';
${imports}

export default function ProductionGovernancePage() {
  return (
    <div className="min-h-screen bg-slate-950 p-6">
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-slate-100 tracking-tight">Enterprise Production Governance</h1>
        <p className="text-slate-400 text-sm mt-1">Constitutional Hardening & Certification Command Center</p>
      </div>
...
if (fs.existsSync('src/app/api/production')) {
  const dirs = fs.readdirSync('src/app/api/production');
  for (const dir of dirs) {
    if (!allGeneratedApis.has(dir) && fs.statSync(path.join('src/app/api/production', dir)).isDirectory()) {
      fs.rmSync(path.join('src/app/api/production', dir), { recursive: true, force: true });
      console.log(\`Cleaned up legacy API route: \${dir}\`);
    }
  }
}
console.log("Phase 219 files generated successfully.");
