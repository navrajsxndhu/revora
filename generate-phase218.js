...
// 1. Generate Engines
const enginesDir = path.join('src', 'lib', 'deployment');
mkdirp(enginesDir);
...
for (const api of apis) {
  const apiDir = path.join('src', 'app', 'api', 'deployment', api);
  mkdirp(apiDir);
...
// 3. Generate Widgets
const widgetsDir = path.join('src', 'components', 'mission-control', 'deployment');
mkdirp(widgetsDir);
...
// 4. Generate Page
const pageDir = path.join('src', 'app', 'mission-control', 'deployment');
mkdirp(pageDir);
const imports = widgets.map(w => `import { ${w} } from '@/components/mission-control/deployment/${w}';`).join('\n');
const components = widgets.map(w => `<${w} />`).join('\n        ');

const pageContent = `import React from 'react';
${imports}

export default function DeploymentGovernancePage() {
  return (
    <div className="min-h-screen bg-slate-950 p-6">
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-slate-100 tracking-tight">Enterprise Deployment Governance</h1>
        <p className="text-slate-400 text-sm mt-1">Constitutional Installer & Bundle Command Center</p>
      </div>
...
if (fs.existsSync('src/app/api/deployment')) {
  const dirs = fs.readdirSync('src/app/api/deployment');
  for (const dir of dirs) {
    if (!allGeneratedApis.has(dir) && fs.statSync(path.join('src/app/api/deployment', dir)).isDirectory()) {
      fs.rmSync(path.join('src/app/api/deployment', dir), { recursive: true, force: true });
      console.log(\`Cleaned up legacy API route: \${dir}\`);
    }
  }
}
console.log("Phase 218 files generated successfully.");
