...
// 1. Generate Engines
const enginesDir = path.join('src', 'lib', 'developer');
mkdirp(enginesDir);
...
for (const api of apis) {
  const apiDir = path.join('src', 'app', 'api', 'developer', api);
  mkdirp(apiDir);
...
// 3. Generate Widgets
const widgetsDir = path.join('src', 'components', 'mission-control', 'developer');
mkdirp(widgetsDir);
...
// 4. Generate Page
const pageDir = path.join('src', 'app', 'mission-control', 'developer');
mkdirp(pageDir);
const imports = widgets.map(w => `import { ${w} } from '@/components/mission-control/developer/${w}';`).join('\n');
...
export default function DeveloperGovernancePage() {
  return (
    <div className="min-h-screen bg-slate-950 p-6">
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-slate-100 tracking-tight">Enterprise Developer Governance</h1>
        <p className="text-slate-400 text-sm mt-1">Constitutional Toolchain & SDK Command Center</p>
      </div>
...
if (fs.existsSync('src/app/api/developer')) {
  const dirs = fs.readdirSync('src/app/api/developer');
  for (const dir of dirs) {
    if (!allGeneratedApis.has(dir) && fs.statSync(path.join('src/app/api/developer', dir)).isDirectory()) {
      fs.rmSync(path.join('src/app/api/developer', dir), { recursive: true, force: true });
      console.log(\`Cleaned up legacy API route: \${dir}\`);
    }
  }
}
console.log("Phase 217 files generated successfully.");
