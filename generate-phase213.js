...
// 1. Generate Engines
const enginesDir = path.join('src', 'lib', 'resources');
mkdirp(enginesDir);
...
for (const api of apis) {
  const apiDir = path.join('src', 'app', 'api', 'resources', api);
  mkdirp(apiDir);
...
// 3. Generate Widgets
const widgetsDir = path.join('src', 'components', 'mission-control', 'resources');
mkdirp(widgetsDir);
...
// 4. Generate Page
const pageDir = path.join('src', 'app', 'mission-control', 'resources');
mkdirp(pageDir);
const imports = widgets.map(w => `import { ${w} } from '@/components/mission-control/resources/${w}';`).join('\n');
...
console.log("Phase 213 files generated successfully.");
