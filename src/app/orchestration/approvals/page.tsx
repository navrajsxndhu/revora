      ...
      <div className="max-w-7xl mx-auto space-y-8 flex flex-col h-[calc(100vh-4rem)]">
        ...
        <header className="flex items-end justify-between border-b border-slate-900 pb-6 shrink-0">
          <div>
            ...
            <p className="text-slate-400">Coordinates mandatory human decision points with escalations, overrides, and strict separation of duties.</p>
          </div>
        ...
        <div className="grid grid-cols-4 gap-4 shrink-0">
          ...
          <div className="bg-slate-900/60 border border-indigo-900/30 bg-indigo-950/10 shadow-[0_0_15px_rgba(129,140,248,0.05)] rounded-xl p-5">
            ...
            <div className="text-xs text-indigo-400">Across enterprise</div>
          </div>
          ...
          <div className="bg-slate-900/60 border border-slate-800 rounded-xl p-5">
            ...
            <div className="text-xs text-slate-500">Executive emergency</div>
          </div>
        </div>
        ...
        <div className="flex-1 min-h-0 pb-12 flex flex-col gap-6">
          ...
          >
            {[{"req":"Deploy Prod DB Cluster","wf":"Infrastructure Change","reqp":"Lead Architect","wait":"14m","status":"Pending","trace":"HAO-EV-301"},{"req":"Access to SOC2 Audit Logs","wf":"Privileged Access","reqp":"Compliance Officer","wait":"2h 10m","status":"Pending","trace":"HAO-EV-302"},{"req":"Emergency Security Patch","wf":"Incident Response","reqp":"CISO (Overridden)","wait":"2m","status":"Approved","trace":"HAO-EV-303"}].map((row, i) => (
              <tr key={i} className="hover:bg-slate-800/30 transition-colors group cursor-pointer border-b border-slate-800/50">
                ...
                <td className="py-4 px-5">
                  <span className={`px-2 py-1 rounded text-xs font-bold border flex items-center gap-1 w-max ${
                    row.status === 'Critical' || row.status === 'High' || row.status === 'Blocked' || row.status === 'Failed' ? 'bg-rose-900/20 text-rose-400 border-rose-900/50' : 
                    row.status === 'Warning' || row.status === 'Medium' || row.status === 'Pending' || row.status === 'Simulating' ? 'bg-amber-900/20 text-amber-400 border-amber-900/50' :
                    row.status === 'Optimal' || row.status === 'Active' || row.status === 'Approved' || row.status === 'Verified' || row.status === 'Live' || row.status === 'Executed' ? 'bg-emerald-900/20 text-emerald-400 border-emerald-900/50' :
