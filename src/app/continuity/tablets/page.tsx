      ...
      <div className="max-w-7xl mx-auto space-y-8 flex flex-col h-[calc(100vh-4rem)]">
        ...
        <header className="flex items-end justify-between border-b border-slate-900 pb-6 shrink-0">
          <div>
            ...
            <h1 className="text-3xl font-bold tracking-tight text-white mb-2 flex items-center gap-3">
              <Tablet className="w-8 h-8 text-indigo-500" />
              Tablet Productivity Studio
            </h1>
            <p className="text-slate-400">Transforms tablets into full enterprise workstations with split-view and stylus interactions.</p>
          </div>
        ...
        <div className="grid grid-cols-4 gap-4 shrink-0">
          ...
          <div className="bg-slate-900/60 border border-indigo-900/30 bg-indigo-950/10 shadow-[0_0_15px_rgba(99,102,241,0.05)] rounded-xl p-5">
            ...
            <div className="text-xs text-indigo-400">Active daily users</div>
          </div>
          <div className="bg-slate-900/60 border border-slate-800 rounded-xl p-5">
            ...
            <div className="text-xs text-slate-500">Multi-tasking adoption</div>
          </div>
          <div className="bg-slate-900/60 border border-slate-800 rounded-xl p-5">
            <div className="text-slate-400 text-sm font-medium mb-1 flex items-center justify-between">
              Stylus Interactions
            ...
            <div className="text-xs text-slate-500">Signatures & annotations</div>
          </div>
          <div className="bg-slate-900/60 border border-slate-800 rounded-xl p-5">
            <div className="text-slate-400 text-sm font-medium mb-1 flex items-center justify-between">
              Landscape Mode
            ...
            <div className="text-xs text-slate-500">Orientation preference</div>
          </div>
        </div>
        ...
        <div className="flex-1 min-h-0 pb-12 flex flex-col gap-6">
          ...
          >
            {[{"role":"Field Auditor","orient":"Portrait","pat":"Checklist + Camera","input":"Touch + Stylus","gov":"Verified","trace":"TPS-EV-301"},{"role":"Financial Analyst","orient":"Landscape","pat":"Multi-column Data Grids","input":"Keyboard + Trackpad","gov":"Verified","trace":"TPS-EV-302"},{"role":"Executive","orient":"Landscape","pat":"Presentation Mode","input":"Touch","gov":"Verified","trace":"TPS-EV-303"}].map((row, i) => (
              <tr key={i} className="hover:bg-slate-800/30 transition-colors group cursor-pointer border-b border-slate-800/50">
                ...
                <td className="py-4 px-5">
                  <span className={`px-2 py-1 rounded text-xs font-bold border flex items-center gap-1 w-max ${
                    row.gov === 'Critical' || row.gov === 'Offline' || row.gov === 'Blocked' || row.gov === 'Failed' ? 'bg-rose-900/20 text-rose-400 border-rose-900/50' : 
                    row.gov === 'Syncing' || row.gov === 'Medium' || row.gov === 'Pending validation' || row.gov === 'Cached' ? 'bg-amber-900/20 text-amber-400 border-amber-900/50' :
                    row.gov === 'Optimal' || row.gov === 'Live' || row.gov === 'Approved' || row.gov === 'Verified' || row.gov === 'Seamless' || row.gov === 'Routed' ? 'bg-emerald-900/20 text-emerald-400 border-emerald-900/50' :
