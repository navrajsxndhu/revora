...
export default async function Page() {
  ...
  const TABLE_DATA = await ObservabilityService.getTelemetry(workspaceId, session.user.id, session.user.role);
    ...
    <PageShell>
      ...
      <div className="flex-1 min-h-0 pb-12 flex flex-col gap-6">
        <PremiumTable title="Operational Telemetry" headers={["Telemetry Source", "Signal Type", "Volume / Rate", "Business Domain", "State", "Trace"]}>
          {TABLE_DATA.length === 0 ? (
            <tr><td colSpan={6} className="py-8 text-center text-slate-500">No telemetry data available.</td></tr>
            ...
            <tr key={row.id || i} className="hover:bg-slate-800/30 transition-colors duration-200 cursor-pointer border-b border-slate-800/50">
                ...
                <td className="py-4 px-5 text-sm text-slate-400">{row.volume}</td>
