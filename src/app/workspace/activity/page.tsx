      ...
      <div className="max-w-7xl mx-auto space-y-8 flex flex-col h-[calc(100vh-4rem)]">
        ...
        <header className="flex items-end justify-between border-b border-slate-900 pb-6 shrink-0">
          <div>
            ...
            <h1 className="text-3xl font-bold tracking-tight text-white mb-2 flex items-center gap-3">
              <ActivitySquare className="w-8 h-8 text-fuchsia-500" />
              Enterprise Activity Feed
            </h1>
            <p className="text-slate-400">The real-time pulse of the organization prioritized by constitutional importance.</p>
          </div>
        ...
        <div className="grid grid-cols-4 gap-4 shrink-0">
          ...
          <div className="bg-slate-900/60 border border-fuchsia-900/30 bg-fuchsia-950/10 shadow-[0_0_15px_rgba(217,70,239,0.05)] rounded-xl p-5">
            ...
            <div className="text-xs text-fuchsia-400">Monitored systems</div>
          </div>
          <div className="bg-slate-900/60 border border-slate-800 rounded-xl p-5">
            ...
            <div className="text-xs text-slate-500">Irrelevant alerts filtered</div>
          </div>
          <div className="bg-slate-900/60 border border-slate-800 rounded-xl p-5">
            <div className="text-slate-400 text-sm font-medium mb-1 flex items-center justify-between">
              Processing Delay
            ...
            <div className="text-xs text-slate-500">Event evaluation time</div>
          </div>
          <div className="bg-slate-900/60 border border-slate-800 rounded-xl p-5">
            <div className="text-slate-400 text-sm font-medium mb-1 flex items-center justify-between">
              Executive Handoff
            ...
            <div className="text-xs text-slate-500">Escalations delivered</div>
          </div>
        </div>
        ...
        <div className="flex-1 min-h-0 pb-12 flex flex-col gap-6">
          ...
          >
            {[{"str":"Infrastructure Reliability","urg":"Critical","sum":"Primary DB Latency Spike Detected","dept":"SRE / Cloud","status":"Active","trace":"EAF-EV-201"},{"str":"AI Governance","urg":"Medium","sum":"New Model Version Evaluated","dept":"Data Science","status":"Pending","trace":"EAF-EV-202"},{"str":"Marketplace Operations","urg":"Low","sum":"Weekly Plugin Sync Finished","dept":"Platform","status":"Completed","trace":"EAF-EV-203"}].map((row, i) => (
              <tr key={i} className="hover:bg-slate-800/30 transition-colors group cursor-pointer border-b border-slate-800/50">
                ...
                <td className="py-4 px-5">
                  <span className={`px-2 py-1 rounded text-xs font-bold border flex items-center gap-1 w-max ${
                    row.status === 'Critical' || row.status === 'High' || row.status === 'Failed' || row.status === 'Overdue' ? 'bg-rose-900/20 text-rose-400 border-rose-900/50' : 
                    row.status === 'Warning' || row.status === 'Medium' || row.status === 'Pending' || row.status === 'Active' ? 'bg-amber-900/20 text-amber-400 border-amber-900/50' :
                    row.status === 'Optimal' || row.status === 'Low' || row.status === 'Approved' || row.status === 'Healthy' || row.status === 'Resolved' || row.status === 'Completed' ? 'bg-emerald-900/20 text-emerald-400 border-emerald-900/50' :
