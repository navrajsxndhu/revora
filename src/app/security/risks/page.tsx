...
export default async function Page() {
  ...
  const TABLE_DATA = await SecurityService.getRisks(workspaceId, session.user.id, session.user.role);
  ...
  return (
    <PageShell>
      ...
      <div className="flex-1 min-h-0 pb-12 flex flex-col gap-6">
        <PremiumTable title="Risk Register" headers={["Risk Name", "Impact", "Likelihood", "Score", "Created At"]}>
          {TABLE_DATA.length === 0 ? (
