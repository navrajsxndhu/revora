...
export default async function Page() {
  ...
  const TABLE_DATA = await SecurityService.getSecurityAnalytics(workspaceId, session.user.id, session.user.role);
  ...
  return (
    <PageShell>
      ...
      <div className="flex-1 min-h-0 pb-12 flex flex-col gap-6">
        <PremiumTable title="Security Analytics" headers={["Metric Name", "Value", "Trend", "Period", "Created At"]}>
          {TABLE_DATA.length === 0 ? (
