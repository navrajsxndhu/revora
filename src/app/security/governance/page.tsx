...
export default async function Page() {
  ...
  const TABLE_DATA = await SecurityService.getSecurityGovernance(workspaceId, session.user.id, session.user.role);
  ...
  return (
    <PageShell>
      ...
      <div className="flex-1 min-h-0 pb-12 flex flex-col gap-6">
        <PremiumTable title="Security Governance" headers={["Control Name", "Framework", "Owner", "Status", "Created At"]}>
          {TABLE_DATA.length === 0 ? (
