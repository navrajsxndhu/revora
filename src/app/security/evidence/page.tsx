...
export default async function Page() {
  ...
  const TABLE_DATA = await SecurityService.getSecurityEvidence(workspaceId, session.user.id, session.user.role);
  ...
  return (
    <PageShell>
      ...
      <div className="flex-1 min-h-0 pb-12 flex flex-col gap-6">
        <PremiumTable title="Security Evidence" headers={["Evidence ID", "Description", "Hash", "Timestamp"]}>
          {TABLE_DATA.length === 0 ? (
