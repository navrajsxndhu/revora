      ...
      <div className="max-w-7xl mx-auto space-y-8 flex flex-col h-[calc(100vh-4rem)]">
        ...
        <header className="flex items-end justify-between border-b border-slate-900 pb-6 shrink-0">
          <div>
            ...
            <p className="text-slate-400">High-priority organizational announcements with verifiable delivery tracking and acknowledgement.</p>
          </div>
        ...
        <div className="grid grid-cols-4 gap-4 shrink-0">
          ...
          <div className="bg-slate-900/60 border border-cyan-900/30 bg-cyan-950/10 shadow-[0_0_15px_rgba(6,182,212,0.05)] rounded-xl p-5">
            ...
            <div className="text-xs text-cyan-400">Enterprise wide announcements</div>
          </div>
          <div className="bg-slate-900/60 border border-slate-800 rounded-xl p-5">
            ...
            <div className="text-xs text-slate-500">Verified reading completion</div>
          </div>
          <div className="bg-slate-900/60 border border-slate-800 rounded-xl p-5">
            <div className="text-slate-400 text-sm font-medium mb-1 flex items-center justify-between">
              Noise Reduction
            ...
            <div className="text-xs text-slate-500">Compared to email blasts</div>
          </div>
          <div className="bg-slate-900/60 border border-slate-800 rounded-xl p-5">
            <div className="text-slate-400 text-sm font-medium mb-1 flex items-center justify-between">
              Mandatory Reads
        ...
        <div className="flex-1 min-h-0 pb-12 flex flex-col gap-6">
          ...
          >
            {[{"title":"Critical Security Patch: Mandatory Restart","auth":"CISO","aud":"Global (All)","ack":"94%","gov":"Approved","trace":"EBG-EV-501"},{"title":"Updated PTO Policy 2027","auth":"VP HR","aud":"Global (All)","ack":"88%","gov":"Approved","trace":"EBG-EV-502"},{"title":"New CRM Vendor Evaluation","auth":"Sales Ops","aud":"Sales Dept","ack":"100%","gov":"Completed","trace":"EBG-EV-503"}].map((row, i) => (
              <tr key={i} className="hover:bg-slate-800/30 transition-colors group cursor-pointer border-b border-slate-800/50">
                ...
                <td className="py-4 px-5">
                  <span className={`px-2 py-1 rounded text-xs font-bold border flex items-center gap-1 w-max ${
                    row.gov === 'Offline' || row.gov === 'Do Not Disturb' || row.gov === 'Restricted' || row.gov === 'Declined' ? 'bg-rose-900/20 text-rose-400 border-rose-900/50' : 
                    row.gov === 'Focus Mode' || row.gov === 'In Meeting' || row.gov === 'Pending Review' || row.gov === 'In Progress' ? 'bg-amber-900/20 text-amber-400 border-amber-900/50' :
                    row.gov === 'Available' || row.gov === 'Active' || row.gov === 'Approved' || row.gov === 'Completed' ? 'bg-emerald-900/20 text-emerald-400 border-emerald-900/50' :
