      ...
      <div className="max-w-7xl mx-auto space-y-8 flex flex-col h-[calc(100vh-4rem)]">
        ...
        <header className="flex items-end justify-between border-b border-slate-900 pb-6 shrink-0">
          <div>
            ...
            <h1 className="text-3xl font-bold tracking-tight text-white mb-2 flex items-center gap-3">
              <Waypoints className="w-8 h-8 text-teal-500" />
              Enterprise Dependency Explorer
            </h1>
            <p className="text-slate-400">Provides a complete top-to-bottom dependency map for every governed object in the organization.</p>
          </div>
        ...
        <div className="grid grid-cols-4 gap-4 shrink-0">
          ...
          <div className="bg-slate-900/60 border border-teal-900/30 bg-teal-950/10 shadow-[0_0_15px_rgba(20,184,166,0.05)] rounded-xl p-5">
            ...
            <div className="text-xs text-teal-400">Tier-1 dependencies</div>
          </div>
          <div className="bg-slate-900/60 border border-slate-800 rounded-xl p-5">
            ...
            <div className="text-xs text-slate-500">Redundancy verified</div>
          </div>
          <div className="bg-slate-900/60 border border-slate-800 rounded-xl p-5">
            <div className="text-slate-400 text-sm font-medium mb-1 flex items-center justify-between">
              Dependency Depth
            ...
            <div className="text-xs text-slate-500">Average graph traversal</div>
          </div>
          <div className="bg-slate-900/60 border border-slate-800 rounded-xl p-5">
            <div className="text-slate-400 text-sm font-medium mb-1 flex items-center justify-between">
              Circular Dep.
            ...
            <div className="text-3xl font-bold text-white mb-2">0</div>
            <div className="text-xs text-slate-500">Architectural debt</div>
          </div>
        </div>
        ...
        <div className="flex-1 min-h-0 pb-12 flex flex-col gap-6">
          ...
          >
            {[{"name":"Auth Service","up":"2 (Identity Provider, Cache)","down":"142 Internal Apps","own":"Platform Security","risk":"Critical","trace":"EDE-EV-301"},{"name":"Financial Ledger","up":"4 (ERP, Billing, HRIS)","down":"14 Reporting Dashboards","own":"Finance Tech","risk":"High","trace":"EDE-EV-302"},{"name":"Email Gateway","up":"1 (DNS Provider)","down":"All Employees","own":"IT Operations","risk":"Medium","trace":"EDE-EV-303"}].map((row, i) => (
              <tr key={i} className="hover:bg-slate-800/30 transition-colors group cursor-pointer border-b border-slate-800/50">
                ...
                <td className="py-4 px-5">
                  <span className={`px-2 py-1 rounded text-xs font-bold border flex items-center gap-1 w-max ${
                    row.risk === 'Critical' || row.risk === 'High' || row.risk === 'Severe' || row.risk === 'Blocked' ? 'bg-rose-900/20 text-rose-400 border-rose-900/50' : 
                    row.risk === 'Warning' || row.risk === 'Medium' || row.risk === 'Pending' || row.risk === 'Elevated' ? 'bg-amber-900/20 text-amber-400 border-amber-900/50' :
                    row.risk === 'Optimal' || row.risk === 'Low' || row.risk === 'Approved' || row.risk === 'Verified' || row.risk === 'Strong' || row.risk === 'Healthy' ? 'bg-emerald-900/20 text-emerald-400 border-emerald-900/50' :
