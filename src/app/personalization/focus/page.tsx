      ...
      <div className="max-w-7xl mx-auto space-y-8 flex flex-col h-[calc(100vh-4rem)]">
        ...
        <header className="flex items-end justify-between border-b border-slate-900 pb-6 shrink-0">
          <div>
            ...
            <h1 className="text-3xl font-bold tracking-tight text-white mb-2 flex items-center gap-3">
              ...
              Productivity Focus Engine
            </h1>
            <p className="text-slate-400">Eliminates noise by adapting the entire Enterprise UI to match the user's current objective.</p>
          </div>
        ...
        <div className="grid grid-cols-4 gap-4 shrink-0">
          ...
          <div className="bg-slate-900/60 border border-cyan-900/30 bg-cyan-950/10 shadow-[0_0_15px_rgba(6,182,212,0.05)] rounded-xl p-5">
            ...
            <div className="text-3xl font-bold text-white mb-2">42K</div>
            <div className="text-xs text-cyan-400">Weekly deep work</div>
          </div>
          <div className="bg-slate-900/60 border border-slate-800 rounded-xl p-5">
            ...
            <div className="text-xs text-slate-500">Uninterrupted time</div>
          </div>
          <div className="bg-slate-900/60 border border-slate-800 rounded-xl p-5">
            <div className="text-slate-400 text-sm font-medium mb-1 flex items-center justify-between">
              Alerts Suppressed
            ...
            <div className="text-xs text-slate-500">Distractions blocked</div>
          </div>
          <div className="bg-slate-900/60 border border-slate-800 rounded-xl p-5">
            <div className="text-slate-400 text-sm font-medium mb-1 flex items-center justify-between">
              Task Completion
            ...
            <div className="text-xs text-slate-500">Versus standard mode</div>
          </div>
        </div>
        ...
        <div className="flex-1 min-h-0 pb-12 flex flex-col gap-6">
          ...
          >
            {[{"prof":"Incident Response Mode","supp":"Social, HR, Routine Tasks","esc":"P1 Alerts Only","act":"142","level":"Deep Focus","trace":"PFE-EV-401"},{"prof":"Executive Review","supp":"Chat, Minor Notifications","esc":"C-Suite Approvals","act":"18","level":"High","trace":"PFE-EV-402"},{"prof":"Standard Work","supp":"None","esc":"Standard SLA Rules","act":"14,020","level":"Medium","trace":"PFE-EV-403"}].map((row, i) => (
              <tr key={i} className="hover:bg-slate-800/30 transition-colors group cursor-pointer border-b border-slate-800/50">
                ...
                <td className="py-4 px-5">
                  <span className={`px-2 py-1 rounded text-xs font-bold border flex items-center gap-1 w-max ${
                    row.level === 'Critical' || row.level === 'Low' || row.level === 'Blocked' || row.level === 'Failed' ? 'bg-rose-900/20 text-rose-400 border-rose-900/50' : 
                    row.level === 'Warning' || row.level === 'Medium' || row.level === 'Review Required' ? 'bg-amber-900/20 text-amber-400 border-amber-900/50' :
                    row.level === 'Optimal' || row.level === 'High' || row.level === 'Approved' || row.level === 'Verified' || row.level === 'Deep Focus' ? 'bg-emerald-900/20 text-emerald-400 border-emerald-900/50' :
