      ...
      <div className="max-w-7xl mx-auto space-y-8 flex flex-col h-[calc(100vh-4rem)]">
        ...
        <header className="flex items-end justify-between border-b border-slate-900 pb-6 shrink-0">
          <div>
            ...
            <p className="text-slate-400">Quantifies the exact time and cognitive load saved by eliminating traditional page-based navigation.</p>
          </div>
        ...
        <div className="grid grid-cols-4 gap-4 shrink-0">
          ...
          <div className="bg-slate-900/60 border border-indigo-900/30 bg-indigo-950/10 shadow-[0_0_15px_rgba(129,140,248,0.05)] rounded-xl p-5">
            ...
            <div className="text-xs text-indigo-400">Workspace efficiency</div>
          </div>
          <div className="bg-slate-900/60 border border-slate-800 rounded-xl p-5">
            ...
            <div className="text-xs text-slate-500">Fewer full-page loads</div>
          </div>
          <div className="bg-slate-900/60 border border-slate-800 rounded-xl p-5">
            <div className="text-slate-400 text-sm font-medium mb-1 flex items-center justify-between">
              Investigation Speed
            ...
            <div className="text-xs text-slate-500">Faster resolution</div>
          </div>
          <div className="bg-slate-900/60 border border-slate-800 rounded-xl p-5">
            <div className="text-slate-400 text-sm font-medium mb-1 flex items-center justify-between">
              Context Loss
            ...
            <div className="text-3xl font-bold text-white mb-2">Near 0</div>
            <div className="text-xs text-slate-500">Multi-panel benefit</div>
          </div>
        </div>
        ...
        <div className="flex-1 min-h-0 pb-12 flex flex-col gap-6">
          ...
          >
            {[{"dept":"Engineering Ops","trad":"14,204","ws":"412","time":"1,420","cond":"Optimal","trace":"WPA-EV-701"},{"dept":"Compliance & Audit","trad":"8,401","ws":"84","time":"840","cond":"Optimal","trace":"WPA-EV-702"},{"dept":"Executive Suite","trad":"1,420","ws":"14","time":"142","cond":"Optimal","trace":"WPA-EV-703"}].map((row, i) => (
              <tr key={i} className="hover:bg-slate-800/30 transition-colors group cursor-pointer border-b border-slate-800/50">
                ...
                <td className="py-4 px-5">
                  <span className={`px-2 py-1 rounded text-xs font-bold border flex items-center gap-1 w-max ${
                    row.cond === 'Critical' || row.cond === 'High' || row.cond === 'Offline' || row.cond === 'Blocked' ? 'bg-rose-900/20 text-rose-400 border-rose-900/50' : 
                    row.cond === 'Warning' || row.cond === 'Medium' || row.cond === 'Background' || row.cond === 'Suspended' ? 'bg-amber-900/20 text-amber-400 border-amber-900/50' :
                    row.cond === 'Optimal' || row.cond === 'Active' || row.cond === 'Foreground' || row.cond === 'Verified' || row.cond === 'Live' || row.cond === 'Docked' ? 'bg-emerald-900/20 text-emerald-400 border-emerald-900/50' :
