      ...
      <div className="max-w-7xl mx-auto space-y-8 flex flex-col h-[calc(100vh-4rem)]">
        ...
        <header className="flex items-end justify-between border-b border-slate-900 pb-6 shrink-0">
          <div>
            ...
            <h1 className="text-3xl font-bold tracking-tight text-white mb-2 flex items-center gap-3">
              <Menu className="w-8 h-8 text-indigo-400" />
              Universal Enterprise Dock
            </h1>
            <p className="text-slate-400">Provides instant OS-style access to favorites, running modules, and active approvals.</p>
          </div>
        ...
        <div className="grid grid-cols-4 gap-4 shrink-0">
          ...
          <div className="bg-slate-900/60 border border-indigo-900/30 bg-indigo-950/10 shadow-[0_0_15px_rgba(129,140,248,0.05)] rounded-xl p-5">
            ...
            <div className="text-xs text-indigo-400">Daily launches</div>
          </div>
          <div className="bg-slate-900/60 border border-slate-800 rounded-xl p-5">
            ...
            <div className="text-3xl font-bold text-white mb-2">9.4</div>
            <div className="text-xs text-slate-500">Avg per user</div>
          </div>
          <div className="bg-slate-900/60 border border-slate-800 rounded-xl p-5">
            <div className="text-slate-400 text-sm font-medium mb-1 flex items-center justify-between">
              Nav Clicks Saved
            ...
            <div className="text-xs text-slate-500">Compared to menus</div>
          </div>
          <div className="bg-slate-900/60 border border-slate-800 rounded-xl p-5">
            <div className="text-slate-400 text-sm font-medium mb-1 flex items-center justify-between">
              Dock Position
            ...
            <div className="text-3xl font-bold text-white mb-2">Dynamic</div>
            <div className="text-xs text-slate-500">Bottom / Left Edge</div>
          </div>
        </div>
        ...
        <div className="flex-1 min-h-0 pb-12 flex flex-col gap-6">
          ...
          >
            {[{"item":"Incident Response Center","type":"System Dashboard","vel":"40ms","freq":"High (42/day)","status":"Docked","trace":"UED-EV-201"},{"item":"Q3 Budget Approval","type":"Active Approval","vel":"14ms","freq":"Medium","status":"Active","trace":"UED-EV-202"},{"item":"Search Graph","type":"Core Utility","vel":"5ms","freq":"Very High","status":"Docked","trace":"UED-EV-203"}].map((row, i) => (
              <tr key={i} className="hover:bg-slate-800/30 transition-colors group cursor-pointer border-b border-slate-800/50">
                ...
                <td className="py-4 px-5">
                  <span className={`px-2 py-1 rounded text-xs font-bold border flex items-center gap-1 w-max ${
                    row.status === 'Critical' || row.status === 'High' || row.status === 'Offline' || row.status === 'Blocked' ? 'bg-rose-900/20 text-rose-400 border-rose-900/50' : 
                    row.status === 'Warning' || row.status === 'Medium' || row.status === 'Background' || row.status === 'Suspended' ? 'bg-amber-900/20 text-amber-400 border-amber-900/50' :
                    row.status === 'Optimal' || row.status === 'Active' || row.status === 'Foreground' || row.status === 'Verified' || row.status === 'Live' || row.status === 'Docked' ? 'bg-emerald-900/20 text-emerald-400 border-emerald-900/50' :
