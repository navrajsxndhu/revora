      ...
      <div className="max-w-7xl mx-auto space-y-8 flex flex-col h-[calc(100vh-4rem)]">
        ...
        <header className="flex items-end justify-between border-b border-slate-900 pb-6 shrink-0">
          <div>
            ...
            <h1 className="text-3xl font-bold tracking-tight text-white mb-2 flex items-center gap-3">
              <HeartHandshake className="w-8 h-8 text-cyan-500" />
              Organization Confidence Center
            </h1>
            <p className="text-slate-400">Measuring the psychological safety and transparency levels across the enterprise.</p>
          </div>
        ...
        <div className="grid grid-cols-4 gap-4 shrink-0">
          ...
          <div className="bg-slate-900/60 border border-cyan-900/30 bg-cyan-950/10 shadow-[0_0_15px_rgba(6,182,212,0.05)] rounded-xl p-5">
            ...
            <div className="text-xs text-cyan-400">Enterprise aggregate score</div>
          </div>
          <div className="bg-slate-900/60 border border-slate-800 rounded-xl p-5">
            ...
            <div className="text-xs text-slate-500">Users understanding rules</div>
          </div>
          <div className="bg-slate-900/60 border border-slate-800 rounded-xl p-5">
            <div className="text-slate-400 text-sm font-medium mb-1 flex items-center justify-between">
              Fear Reduction
            ...
            <div className="text-xs text-slate-500">Hesitation to act</div>
          </div>
          <div className="bg-slate-900/60 border border-slate-800 rounded-xl p-5">
            <div className="text-slate-400 text-sm font-medium mb-1 flex items-center justify-between">
              Shadow IT
            ...
            <div className="text-3xl font-bold text-white mb-2">Eliminated</div>
            <div className="text-xs text-slate-500">Trust removes workarounds</div>
          </div>
        </div>
        ...
        <div className="flex-1 min-h-0 pb-12 flex flex-col gap-6">
          ...
          >
            {[{"dept":"Engineering","trans":"96/100","comp":"High","fric":"Low","status":"Optimal","trace":"OCC-EV-601"},{"dept":"Sales","trans":"88/100","comp":"Medium","fric":"Medium","status":"Optimal","trace":"OCC-EV-602"},{"dept":"Finance","trans":"94/100","comp":"High","fric":"Low","status":"Optimal","trace":"OCC-EV-603"}].map((row, i) => (
              <tr key={i} className="hover:bg-slate-800/30 transition-colors group cursor-pointer border-b border-slate-800/50">
                ...
                <td className="py-4 px-5">
                  <span className={`px-2 py-1 rounded text-xs font-bold border flex items-center gap-1 w-max ${
                    row.status === 'Critical' || row.status === 'Denied' || row.status === 'Low' || row.status === 'Failed' ? 'bg-rose-900/20 text-rose-400 border-rose-900/50' : 
                    row.status === 'Warning' || row.status === 'Pending' || row.status === 'Review Required' || row.status === 'Medium' ? 'bg-amber-900/20 text-amber-400 border-amber-900/50' :
                    row.status === 'Optimal' || row.status === 'Granted' || row.status === 'Approved' || row.status === 'Verified' || row.status === 'High' ? 'bg-emerald-900/20 text-emerald-400 border-emerald-900/50' :
