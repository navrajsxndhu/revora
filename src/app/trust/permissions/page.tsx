      ...
      <div className="max-w-7xl mx-auto space-y-8 flex flex-col h-[calc(100vh-4rem)]">
        ...
        <header className="flex items-end justify-between border-b border-slate-900 pb-6 shrink-0">
          <div>
            ...
            <h1 className="text-3xl font-bold tracking-tight text-white mb-2 flex items-center gap-3">
              <Unlock className="w-8 h-8 text-cyan-500" />
              Permission Transparency Center
            </h1>
            <p className="text-slate-400">Visualizing enterprise access models so users always understand their boundaries.</p>
          </div>
        ...
        <div className="grid grid-cols-4 gap-4 shrink-0">
          ...
          <div className="bg-slate-900/60 border border-cyan-900/30 bg-cyan-950/10 shadow-[0_0_15px_rgba(6,182,212,0.05)] rounded-xl p-5">
            ...
            <div className="text-xs text-cyan-400">Role comprehension</div>
          </div>
          <div className="bg-slate-900/60 border border-slate-800 rounded-xl p-5">
            ...
            <div className="text-xs text-slate-500">Active limited-time access</div>
          </div>
          <div className="bg-slate-900/60 border border-slate-800 rounded-xl p-5">
            <div className="text-slate-400 text-sm font-medium mb-1 flex items-center justify-between">
              Orphaned Roles
            ...
            <div className="text-xs text-slate-500">Continually audited</div>
          </div>
          <div className="bg-slate-900/60 border border-slate-800 rounded-xl p-5">
            <div className="text-slate-400 text-sm font-medium mb-1 flex items-center justify-between">
              Mystery Denials
            ...
            <div className="text-xs text-slate-500">Hidden policies removed</div>
          </div>
        </div>
        ...
        <div className="flex-1 min-h-0 pb-12 flex flex-col gap-6">
          ...
          >
            {[{"res":"SOC2 Compliance Folder","access":"View Only","req":"Auditor (Write)","exp":"Never (Base Role)","trans":"High","trace":"PTC-EV-201"},{"res":"AWS Production Cluster","access":"Full Access (Temporary)","req":"DevOps Lead","exp":"In 4 Hours","trans":"High","trace":"PTC-EV-202"},{"res":"Executive Dashboards","access":"None","req":"VP or Above","exp":"N/A","trans":"High","trace":"PTC-EV-203"}].map((row, i) => (
              <tr key={i} className="hover:bg-slate-800/30 transition-colors group cursor-pointer border-b border-slate-800/50">
                ...
                <td className="py-4 px-5">
                  <span className={`px-2 py-1 rounded text-xs font-bold border flex items-center gap-1 w-max ${
                    row.trans === 'Critical' || row.trans === 'Denied' || row.trans === 'Low' || row.trans === 'Failed' ? 'bg-rose-900/20 text-rose-400 border-rose-900/50' : 
                    row.trans === 'Warning' || row.trans === 'Pending' || row.trans === 'Review Required' || row.trans === 'Medium' ? 'bg-amber-900/20 text-amber-400 border-amber-900/50' :
                    row.trans === 'Optimal' || row.trans === 'Granted' || row.trans === 'Approved' || row.trans === 'Verified' || row.trans === 'High' ? 'bg-emerald-900/20 text-emerald-400 border-emerald-900/50' :
