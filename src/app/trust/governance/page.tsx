      ...
      <div className="max-w-7xl mx-auto space-y-8 flex flex-col h-[calc(100vh-4rem)]">
        ...
        <header className="flex items-end justify-between border-b border-slate-900 pb-6 shrink-0">
          <div>
            ...
            <p className="text-slate-400">Constitutional oversight managing transparency standards and security communication.</p>
          </div>
        ...
        <div className="grid grid-cols-4 gap-4 shrink-0">
          ...
          <div className="bg-slate-900/60 border border-cyan-900/30 bg-cyan-950/10 shadow-[0_0_15px_rgba(6,182,212,0.05)] rounded-xl p-5">
            ...
            <div className="text-xs text-cyan-400">Transparency requirements</div>
          </div>
          <div className="bg-slate-900/60 border border-slate-800 rounded-xl p-5">
            ...
            <div className="text-xs text-slate-500">Awaiting board approval</div>
          </div>
          <div className="bg-slate-900/60 border border-slate-800 rounded-xl p-5">
            <div className="text-slate-400 text-sm font-medium mb-1 flex items-center justify-between">
              Compliance Rate
            ...
            <div className="text-xs text-slate-500">All policies explained</div>
          </div>
          <div className="bg-slate-900/60 border border-slate-800 rounded-xl p-5">
            <div className="text-slate-400 text-sm font-medium mb-1 flex items-center justify-between">
              Opaque Systems
            ...
            <div className="text-xs text-slate-500">Zero black boxes permitted</div>
          </div>
        </div>
        ...
        <div className="flex-1 min-h-0 pb-12 flex flex-col gap-6">
          ...
          >
            {[{"rule":"No Silent Denials","dom":"Platform UI","enf":"Strict (Enforced)","owner":"UX Team","conf":"High","trace":"TGB-EV-801"},{"rule":"Plain English Summaries","dom":"All YAML Policies","enf":"Enforced","owner":"Legal & Sec","conf":"High","trace":"TGB-EV-802"},{"rule":"Visual Expiration Dates","dom":"Temporary Access","enf":"Pending Implementation","owner":"IAM Team","conf":"Medium","trace":"TGB-EV-803"}].map((row, i) => (
              <tr key={i} className="hover:bg-slate-800/30 transition-colors group cursor-pointer border-b border-slate-800/50">
                ...
                <td className="py-4 px-5">
                  <span className={`px-2 py-1 rounded text-xs font-bold border flex items-center gap-1 w-max ${
                    row.conf === 'Critical' || row.conf === 'Denied' || row.conf === 'Low' || row.conf === 'Failed' ? 'bg-rose-900/20 text-rose-400 border-rose-900/50' : 
                    row.conf === 'Warning' || row.conf === 'Pending' || row.conf === 'Review Required' || row.conf === 'Medium' ? 'bg-amber-900/20 text-amber-400 border-amber-900/50' :
                    row.conf === 'Optimal' || row.conf === 'Granted' || row.conf === 'Approved' || row.conf === 'Verified' || row.conf === 'High' ? 'bg-emerald-900/20 text-emerald-400 border-emerald-900/50' :
