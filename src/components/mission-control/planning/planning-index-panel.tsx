...
interface PlanningIndexPanelProps {
  data: {
...
export function PlanningIndexPanel({ data }: PlanningIndexPanelProps) {
  if (!data) return null;

  const isHigh = data.planningScore >= 75;
        ...
        <div className="flex flex-col gap-4">
          ...
          <div className="space-y-2">
            <div className="flex justify-between text-xs">
              <span className="text-slate-400">Planning Maturity</span>
              <span className="text-slate-200 font-medium">{data.planningMaturity.replace(/_/g, ' ')}</span>
            </div>
            <div className="h-2 w-full bg-slate-800 rounded-full overflow-hidden">
              <div 
                className="h-full rounded-full transition-all duration-500 bg-gradient-to-r from-indigo-600 to-indigo-400"
                style={{ width: `${data.planningScore}%` }}
              />
            </div>
          </div>
        </div>
