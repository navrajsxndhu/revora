...
const engines = [
  "installation-engine.ts",
  "bootstrap-engine.ts",
  "provisioning-engine.ts",
...
// 1. Generate Engines
const enginesDir = path.join('src', 'lib', 'installation');
mkdirp(enginesDir);
...
for (const api of apis) {
  const apiDir = path.join('src', 'app', 'api', 'installation', api);
  mkdirp(apiDir);
...
// 3. Generate Widgets
const widgetsDir = path.join('src', 'components', 'mission-control', 'installation');
mkdirp(widgetsDir);
...
// 4. Generate Page
const pageDir = path.join('src', 'app', 'mission-control', 'installation');
mkdirp(pageDir);
const imports = widgets.map(w => `import { ${w} } from '@/components/mission-control/installation/${w}';`).join('\n');
...
console.log("Phase 209 files generated successfully.");
